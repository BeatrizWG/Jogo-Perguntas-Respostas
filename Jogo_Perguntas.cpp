...
#include <ctime> // retorna o tempo atual em segundos 
using namespace std;
...
char mostrar_questao(questao** questoes, int posicao, int pergunta); 
...
void randomizar_valores(int** matrizPosicoes, int colunas, int linhas);
void show();
...
int main(void) {
    ...
    // CRIANDO BIBLIOTECA DE QUESTOES 
    questao** minhas_questoes = criar_biblioteca_questao (colunas, linhas);
    ler_biblioteca(minhas_questoes);

    // CRIANDO ORDEM DAS QUESTOES 
    srand(time(0)); // garantir que a sequência geradas seja diferente em cada execução do programa
                // time(0) para usar o valor de tempo atual, para que a sequência de números seja baseada no valor atual do relógio do sistema
    ...
    //INICIALIZANDO VERIFICACAO DE ERROS DOS JOGADORES
    no* verify_pl1 = NULL;
    no* verify_pl2 = NULL;
    ...
    do
    {
        int pos_atual = definir_vez(num_rodadas, pos_1, pos_2,nome1,nome2); // verifica qual jogador será e retorna sua posição no tabuleiro

        char resposta = '\0';

        // imprimindo a questão e retornando o gabarito 
        if (rodadas % 2 == 0) {
        ...
        if (resposta == resposta_jog) {
            ...
            if (rodadas % 2 == 0) { // fazendo o controle da posição e da pontuação 
                pos_1++;
                pontos_1++;
                indexQ_P1 = 0;
                verify_pl1 = NULL; 
            ...
            if (rodadas % 2 == 0) { //  adicionando mais um erro na lista encadeada
                append(verify_pl1); 
        ...
        if (rodadas % 2 == 0 && pontos_1 == 5) { //verificar se venceu o jogo ou se continua
...
void append(no*& L){
    ...
    // essa função adiciona elementos na lista 
...
int len(no* L){
    ...
    // essa função conta os elementos da lista 
...
void sortear_jogador_inicial (string & nome1, string & nome2){
    ...
    // Essa função vai definir qual jogador inicia jogando

    cout<<"Vamos sortear quem vai jogar primeiro..."<<"\n";
    int pos = (rand () % 2) + 1; // sorteando se o nome1 sera o 1° ou 2°
    if (pos == 1){ // se nome1 for o primeiro, mantém a ordem dos nomes
        cout<<nome1<<" sera o(a) primeiro(a) a jogar "<<"\n";
    } else{ // se nome1 for o segundo, troca a ordem dos nomes 
        string aux = nome1;
        nome1=nome2;
        nome2=aux;
        cout<<nome1<<" sera o(a) primeiro(a) a jogar "<<"\n";
...
int definir_vez(int& num_rodada, int pos1, int pos2,string n1, string n2) {
    ...
    // Essa função verifica quem vai jogar e qual a sua posição 

    int real_pos = -1; 
    if (num_rodada % 2 == 0) {  // definir se é o jogador 1 ou 2
        cout << "--------------------------------------------------------------------------------------------" << "\n";
        cout << "Eh a vez de "<<n1<<" jogar:" <<endl;
        cout << "\n";
        real_pos = pos1; // definir a matéria 
    }
    else {
        cout << "--------------------------------------------------------------------------------------------" << "\n";
...
char mostrar_questao(questao** questoes, int posicao, int pergunta) {
    ...
    // Função para imprimir a questão e suas alternativas 
    // Retorna o gabarito da questõe 
...
int** criar_matriz_randomica(int colunas, int linhas) {
    ...
    // Essa função vai criar uma matriz de inteiros para colocar a ordem sorteada das questões 
...
questao** criar_biblioteca_questao (int colunas, int linhas) {
    ...
    // Funcao para criar uma matriz para cadastrar as perguntas e respostas 
...
void randomizar_valores(int** matriz, int colunas, int linhas) {
    for (int i = 0;i<linhas;i++){
        for (int j =0; j<colunas; j++){
            matriz[i][j]=-1; // retirando o lixo da lista para não dar erro
        }
    }
    int num;
    for (int i = 0;i<linhas;i++){
        for (int j =0; j<colunas; j++){
            do {
                num = rand() % colunas; // gera números aleatórios no intervalo de 0 até o (tamanho - 1)
            } while (elemento_lista(matriz,i,colunas,num)==true); // verificar se tem o elemento na lista e sorteia outro até ele não estar na lista 
            matriz[i][j]=num;
        }
    }
}
...
void ler_biblioteca(questao** mat) {
     ...
     // Nessa função é feito o cadastro de cada questão, com as respectivas alternativas e a resposta
     // Estão separadas da seguinte maneira:
        // [i][j], onde é i representa o tema da questão e j o número da questão 
        // 0 <= j <= 15
